...
NeurOmics Explorer (NEx) is a Shiny-based data explorer for neuromics datasets. The project is split across two runtimes:
...
|   ├── hong.duckdb
|   ├── diaz_castro.duckdb
|   ├── neuromics_registry.duckdb   # Data catalogue contains register, canonical name mapping, indexing, and stats (auto-generated by start-up sequence)
...
2. DuckDB exposes canonical views over the source data and acts as the query engine.
...
│  Python / FastAPI                                   │
│                                                     │
│  • DuckDB connections (pool)                        │
│  • SQL query over registry + Parquet                │
│  • Arrow IPC serialisation/ streaming               │
│  • Gene index lookups                               │
