## This is the UI section for the "Explore Data" page for dataset selection and plot visualisation
...
explorer_ui <- function(id) {
  ns <- NS(id)
  # # Sidebar layout for the entire explore page
  # layout_sidebar(
  #   sidebar = sidebar(
  #     id = ns("explore_sidebar"),
  #     width = 300,
  #     bg = "#f8f9fa",
  
  
  # Main layout with LEFT sidebar for filters and RIGHT column for project info
  fluidRow(
    ...
    # 
    #   # Sidebar title
    #   tags$h4("Data Options", style = "margin-top: 0; color: #667eea;"),
    #   tags$hr(),
  # collapsible but causes annoying scrolling (potentially due to column() below)
  page_sidebar(
    sidebar = sidebar(
    ...
    # Main content column (plots and data)
    # div(
    ...
    column(
      ...
      # Top row: Main visualisation card
      card(
        full_screen = TRUE,
        card_header(
        ...
        card_body(
          ...
          # Tab navigation for exploration and various plots
          navset_card_tab(
            id = ns("exploration_tabs"),
            
            nav_panel(
              title = "Dataset Listings",
              icon = icon("chart-area"),
              # plotlyOutput(ns("main_plot"), height = "600px"),
              dataTableOutput("dataset_listing", height = "600px")
            ),
